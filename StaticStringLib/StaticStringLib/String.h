...
class String {
    ...
    struct FakeRep { 
        static char* sharedEmpty[];
    };

    static sRep* sharedEmpty() {
        return(sRep*)FakeRep::sharedEmpty;
    }
...
public:
    ...
    friend bool operator!=(const String& st1, const char* st2);
    friend bool operator!=(const String& st1, const String& st2);
    //>= > < <= реализуются аналогично этим двум
};
inline String::String() :_rep(sharedEmpty()) {
    ++_rep->_ref;
}
inline String::String(const char* dt) : _rep(new(strlen(dt))sRep) {
    strcpy(_rep->data(), dt);
}
inline String::String(const char* dt, uint n) : _rep(new(n)sRep) {
    strncpy(_rep->data(), dt, n);
}
inline String::String(const String& other) : _rep(other._rep) {
    ++_rep->_ref;
}
...
class StringRef {
    ...
    uint _size;
public:
    StringRef(const String& str, uint pos, uint n) : _str(str), _pos(pos), _size(n) {};
